// ============================================================================
// IMAGE BURNER CONFIGURATION
// ============================================================================
// Edit this file to customize the image burner for your OS project.
//
// QUICK START - To rebrand this burner, change these values:
//   1. APP_NAME        - Your OS name (e.g., "TwigUI", "Onion", "MinUI")
//   2. REPO_OPTIONS    - Your GitHub repositories
//   3. ASSET_EXTENSION - Archive format containing image (.zip, .7z, .gz)
//
...
/// Examples: "TwigUI", "Onion", "MinUI"
pub const APP_NAME: &str = "TwigUI";

/// Volume label - not used for image burning (image contains its own partition table)
/// Kept for legacy compatibility, can be ignored
pub const VOLUME_LABEL: &str = "TWIGUI";
...
// Validate VOLUME_LABEL length (FAT32 limit is 11 characters)
const _: () = {
    const LABEL_LEN: usize = VOLUME_LABEL.len();
    if LABEL_LEN == 0 || LABEL_LEN > 11 {
        panic!("VOLUME_LABEL must be between 1 and 11 characters");
    }
};

// Validate REPO_OPTIONS is not empty
const _: () = {
    if REPO_OPTIONS.len() == 0 {
        panic!("REPO_OPTIONS cannot be empty - at least one repository must be configured");
    }
};

// Validate DEFAULT_REPO_INDEX is within bounds
const _: () = {
    if DEFAULT_REPO_INDEX >= REPO_OPTIONS.len() {
        panic!("DEFAULT_REPO_INDEX is out of range - must be less than REPO_OPTIONS length");
    }
};

// Validate ASSET_EXTENSION starts with a dot
const _: () = {
    if ASSET_EXTENSION.is_empty() {
        panic!("ASSET_EXTENSION cannot be empty");
    }
    let bytes = ASSET_EXTENSION.as_bytes();
    if bytes[0] != b'.' {
        panic!("ASSET_EXTENSION must start with a dot (e.g., \".7z\")");
    }
};
